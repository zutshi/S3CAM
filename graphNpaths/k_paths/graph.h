...
{
  //return o << '(' << a.source << ' ' << a.dest << ' ' << a.weight << ')';//adi:change format; no
  //parens
  return o << a.dest+1; //adi:replace above with this and compensate for matlab's @#$@#$ indexing
}
...
istream & operator >> (istream &istr, GraphArc &a)
{
  char c;
  int i;
//  istr >> c;			// open paren //adi:change format; no parens
  istr >> a.source;
  istr >> a.dest;
  istr >> a.weight;
//  istr >> c;			// close paren //adi:change format; no parens
  a.data = NULL;
  //cout<<a.source<<" "<<a.dest<<" "<<a.weight<<endl;
  return istr;
}
...
template <> Node <GraphArc> *(Node<GraphArc>::freeList) = NULL;//adi
template <> const int Node<GraphArc>::newBlocksize = 64;//adi

template <> Node<int> *Node<int>::freeList = NULL;//adi
template <> const int Node<int>::newBlocksize = 64;//adi
